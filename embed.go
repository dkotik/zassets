...
import "github.com/dkotik/zassets/debug"
...
{{- else -}}
	{{ comment .Comment }}
{{- end }}
var {{ .Variable }} = debug.NewDebugger(
	[]string{ {{- .Entries | join -}} },
	[]string{ {{- .Ignore | join -}} }{{ if .Refine }},
	debug.WithDefaultCompiler(){{ end }})
{{- end -}}
...
func (e *Embed) SetTemplate(t string) {
	e.template = template.New(`default`).Funcs(template.FuncMap{
		"tags": func(tags []string) (string, error) {
			for _, t := range tags {
				if t == `` {
					return ``, fmt.Errorf(`tag setting cannot be set to "%s"`, t)
				}
			}
		...
		"join": func(s []string) string {
			...
			for _, st := range s {
				result = append(result, strconv.Quote(st))
			}
...
func (e *Embed) Reader(w io.Writer, r io.Reader) error {
	...
	if e.Debug {
		// spew.Dump(e)
		return e.template.ExecuteTemplate(w, `debug`, e)
	}
